...
import org.apache.jackrabbit.core.version.IVersioningLock;
 ...
 * @version $Revision: $
 */
public class XIDAwareISMLocking implements IVersioningLock {
	...
	try {
	    ...
	    IReadLock readLock = acceptReader(currentXid, intents.isEmpty());
	...
	return (IReadLock) enqueueLockRequest(new ReadLockRequest(currentXid));
    }
	...
	try {
	    ...
	    IWriteLock writeLock = acceptWriter(currentXid, intents.isEmpty());
	...
	return (IWriteLock) enqueueLockRequest(new WriteLockRequest(currentXid));
    }

    private IReadLock acceptReader(Xid xid, boolean ignoreIntents) {
    ...
    private IWriteLock acceptWriter(Xid xid, boolean ignoreIntents) {
	if (isCurrentlyWriting(xid)) {
    ...
    private class XidHolderReadLock implements IReadLock {
    ...
    private class XidHolderWriteLock implements IWriteLock {
	private final XidLockInfo lockInfo;
	 ...
	 * {@inheritDoc}
	 */
	public IReadLock downgrade() {
	    if (!readers.containsKey(lockInfo.xid)) {
		readers.put(lockInfo.xid, lockInfo);
	    }
	    IReadLock readLock = new XidHolderReadLock(lockInfo);
